...
- **Weave source attributions into the prose.** Instead of `*(Source, date)*` at the end of a bullet, integrate citations naturally: "Mr Zelensky said the two had agreed on post-war security guarantees. (ABC Australia, 2026-01-22)" — parenthetical, after the claim they support.
- **Preserve all source attributions.** Every sourced claim in the input must have its attribution in the output. Do not drop sources. Use the format `(Source Name, YYYY-MM-DD)` — if the source has a URL in the input, use `([Source Name](URL), YYYY-MM-DD)`.
...
- Do not drop source attributions. Every sourced claim must keep its citation.
