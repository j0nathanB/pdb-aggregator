...
"""
    ...
    @with_retry(max_attempts=2)
    async def summarize_and_dedupe(
        self,
        articles: list[Article],
        leader: LeaderConfig,
    ) -> list[Article]:
        """
        Reduce 15-20 articles to 8-12 unique stories by merging duplicates.

        Identifies articles covering the SAME underlying event and merges
        them into unique stories, keeping the most comprehensive version.

        Args:
            articles: Articles to deduplicate
            leader: Leader these articles are about

        Returns:
            Deduplicated list of articles
        """
        if len(articles) <= 8:
            # Already under threshold, no deduplication needed
            return articles

        logger.info(f"Deduplicating {len(articles)} articles for {leader.name}")

        # Build article summaries for LLM
        article_summaries = []
        for i, article in enumerate(articles):
            content = article.display_content[:500] if article.display_content else ""
        ...
        prompt = f"""You are an intelligence analyst consolidating news coverage about {leader.name}.
...
"""
        ...
        if not data or "keep_indices" not in data:
            logger.warning("Failed to parse dedupe response, returning original articles")
            return articles
        ...
        for idx in keep_indices:
            if isinstance(idx, int) and 0 <= idx < len(articles):
                deduped.append(articles[idx])

        # Ensure we have at least some articles
        if not deduped:
            return articles[:12]

        logger.info(f"Deduplicated {len(articles)} to {len(deduped)} articles for {leader.name}")

        return deduped


# System prompt for deduplication
DEDUPE_SYSTEM = """You are an intelligence analyst consolidating news coverage.
