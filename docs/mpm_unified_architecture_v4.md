...
- Full article **extraction** uses a tiered fallback chain: Claude web_fetch → curl+trafilatura → Diffbot Article API → Playwright → Publisher APIs
- **Diffbot** is retained as Tier 2 extraction (complex layouts, metered paywalls), no longer used for discovery or NLP
- **Claude's web_fetch** is retained as Tier 0 extraction (clean HTML), no longer used for discovery by the country agent
...
| External APIs | SearchAPI, Diffbot | Brave Search News API (Layer 1 news discovery), SearchAPI (Layer 2 government discovery), Diffbot (Tier 2 extraction), Publisher APIs (Tier 4 extraction) |
...
|----------|--------|-------------|------|
| Tier 0 | Claude web_fetch | Clean HTML article pages without heavy JavaScript or anti-bot protection | Free (API call) |
| Tier 1 | curl + trafilatura | Default for most domains. Simple HTTP + content extraction. Highest success rate (81%). | Free |
| Tier 2 | Diffbot Article API | Complex layouts, metered paywalls, CMS-embedded content | Per-call |
| Tier 3 | Headless browser (Playwright) | JavaScript-rendered pages, anti-bot protection, sites requiring interaction | Infrastructure |
| Tier 4 | Publisher API | Outlets with structured API access (Guardian Open Platform, FT, others) | Varies by publisher |
...
| Layer 1 extraction: Diffbot calls (Tier 2 fallback, ~30-50 articles/week) | ~$3-5 |
...
- **Diffbot as primary discovery/NLP tool:** Retained as Tier 2 extraction method for articles with complex layouts. No longer used for entity extraction or NLP — the LLM handles analysis.
- **Claude's web_search in the country agent:** The country agent no longer runs searches — discovery is handled by Brave (Layer 1) and SearchAPI (Layer 2). Claude's web_fetch is retained as Tier 0 extraction for clean HTML articles.
- **Playwright for news articles:** Retained as Tier 3 extraction fallback for JS-rendered pages across both layers.
